...
- Flush cache elements matching the key ``block_id = 5``::
    ...
    php bin/console sonata:cache:flush --keys='{"block_id":5}'

- Flush cache elements from Varnish and Memcached matching the key ``block_id = 5``::

    php bin/console sonata:cache:flush --keys='{"block_id":5}' --cache=sonata.page.cache.esi --cache=sonata.cache.memcached
...
- Flush all cache elements from Varnish::
    ...
    php bin/console sonata:cache:flush-all --cache=sonata.page.cache.esi
